 ...
 */
function CronUI(containerSelector, opts) {
    ...
    this.current_value = options.initial;

    // Render the cron form
    this.render();
    
    // Set the form to the initial string if given
    if (typeof options.initial === 'string') {
        this.setCronString(options.initial);
    }
}
...
CronUI.prototype.render = function() {
    ...
    el.querySelector('.cron-period select').addEventListener("change", this.periodChanged.bind(this));
...
CronUI.getCronType = function(cronString) {
    ...
    for (var type in CronUI.cronTypes) {
        if (CronUI.cronTypes[type].test(cronString)) { return type; }
    }
